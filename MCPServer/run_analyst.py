"""
run_analyst.py — интерактивный чат с ИИ-агентом.

Запускает агента один раз и дальше позволяет переписываться с ним в
реальном времени, как с обычным чат-ботом: вводишь сообщение —
получаешь ответ свободным текстом (см. workspace/prompts/analyst.md).

Если в сообщении есть слова "тз" / "техническое задание" / "план
проекта" — оркестратор сам (на уровне кода, не надеясь на модель)
переключается в строгий режим: проверяет, что ответ — валидный JSON
по схеме ТЗ, и при успехе автоматически запускает агента-разработчика
(см. core/orchestrator.py: handle_user_input(..., parse_as_spec=True)).

Выход: команда 'exit', 'quit' или 'выход', либо Ctrl+C.
"""
